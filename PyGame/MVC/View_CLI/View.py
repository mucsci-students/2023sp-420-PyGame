...
def print_current_puzzle():
    prettyGuesses = get_pretty_guesses(PuzzleStats.guesses)
    # variables to store each letter 
    req = stats.shuffled_puzzle[0]
    fir = stats.shuffled_puzzle[3]
    ...
    Puzzle:
        Rank: {stats.get_rank()}   
        Score: {stats.get_score} / {PuzzleStats.get_maxScore} 
...
def print_all_guesses():
    ...
    prettyGuesses = "\t\t"
    counter = 0
    for guess in PuzzleStats.get_guesses:
        counter += 1
        prettyGuesses = prettyGuesses + guess + ", "
        if(counter == 4):
