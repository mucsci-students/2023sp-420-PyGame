        ...
        with open(f'PyGame_Project/MVC/Model/Database/Individual_letters/words_with_{i + 1}.txt', 'r') as file:
            ...
            for j, line in enumerate(lines):
                ...
                if cursor.rowcount == 0:
                    cursor.execute(
                        f"INSERT INTO words ({','.join([chr(97 + k) for k in range(26)])}) VALUES ({','.join(['?' for k in range(26)])})",
                        [None if k != i else line.strip() for k in range(26)])
...
def get_possible_words(cursor, required_letter, word):
    ...
    query = "SELECT {} FROM words WHERE {} GLOB '*{}*' AND {} NOT GLOB '*[^{}]*'".format(required_letter,
                                                                                         required_letter,
                                                                                         required_letter,
                                                                                         required_letter, word)
...
def get_total_points(puzzle_list):
    ...
    for word in puzzle_list:
        ...
        elif length == 7 and len(set(word[0])) == 7:
