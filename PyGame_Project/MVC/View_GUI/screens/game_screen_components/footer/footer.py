...
def create_buttons(state):
    height = 0.0625 * state.display.get_height()
    y_pos = state.display.get_height() - (height * 2.75)
    spacer = height * 3

    left_x_offset = (state.display.get_width() / 6) * .33
    right_x_offset = state.display.get_width() - left_x_offset

    y_pos_top_offset = y_pos - (height * 1.5)
    y_pos_bottom_offset = y_pos + (height * 1.5)
...
def draw_button(state, text, x, y):
    ...
    width = 0.0625 * state.display.get_height()
    hexagon = Hexagon(x, y, width, width, text)
    hexagon.draw(state.display, COLOR_ORANGE, font_color, COLOR_NEON_ORANGE)
    hexagon.draw(state.display, COLOR_BLACK, font_color, COLOR_BLACK, 3)
